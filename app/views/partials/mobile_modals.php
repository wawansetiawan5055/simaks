...
<div class="mobile-drawer" id="mobileGridDrawer">
    ...
    <div class="mobile-drawer-body" style="flex: 1; overflow-y: auto; padding-bottom: 80px;">
                ...
                <div class="modal-footer border-0 p-0 d-flex justify-content-center align-items-center" style="min-height: 40px; background: transparent; flex-shrink: 0; margin-top: -10px;">
...
function fetchGlobalSchedule(callback) {
    const apiUrl = '<?= $api_url ?? "" ?>?mod=api&type=jadwal&act=get_daily';
    if(!apiUrl) return;
...
$(document).ready(function() {
    ...
    function handleTouchEnd(evt) {
        ...
        // Threshold of 30px is slightly more sensitive than 40px, better for 100% zoom
        if (Math.abs(xDiff) > Math.abs(yDiff) && Math.abs(xDiff) > 30) {
            if (xDiff > 0) {
                // Swipe Left -> Next (Let Bootstrap handle wrap based on its internal state)
