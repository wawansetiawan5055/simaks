<?php
// app/views/laporan_mutasi_siswa.php - Laporan Mutasi Siswa with Dual-Tab Inline Paper Studio
    ...
    <div class="row align-items-center">
      <div class="col-sm-6 col-12 d-flex align-items-center">
        ...
        <div>
          ...
          <p class="text-muted small m-0">Rekapitulasi perpindahan peserta didik &amp; cetak berkas arsip mutasi</p>
        </div>
      </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap no-print">
      ...
      <div class="d-flex align-items-center gap-2 mt-2 mt-md-0">
        <span class="badge badge-light border px-3 py-2 text-dark font-weight-bold shadow-sm" style="font-size: 0.88rem;">
          <i class="fas fa-people-arrows text-primary mr-1"></i> Total: <?= count($list ?? []) ?> Mutasi
        </span>
      </div>
    </div>
        ...
        <div class="card shadow-sm border-0">
          <div class="card-header bg-white border-bottom-0 pt-3 pb-2 d-flex justify-content-between align-items-center flex-wrap">
            ...
            <div class="btn-group shadow-sm">
              <a href="<?= BASE_URL ?>laporan/mutasi_siswa_export_excel?<?= $query_string_custom ?>" class="btn btn-success btn-sm font-weight-bold px-3">
                <i class="fas fa-file-excel mr-1"></i> Excel
              </a>
              <a href="<?= BASE_URL ?>laporan/mutasi_siswa_export_pdf?<?= $query_string_custom ?>" class="btn btn-danger btn-sm font-weight-bold px-3" target="_blank">
        ...
        <div class="card shadow-sm border-0 mb-3 bg-white btn-studio-toolbar no-print">
          <div class="card-body p-3 d-flex justify-content-between align-items-center flex-wrap">
            ...
            <div class="btn-group shadow-sm mt-2 mt-md-0">
              ...
              <a href="<?= BASE_URL ?>laporan/mutasi_siswa_export_pdf?<?= $query_string_custom ?>" class="btn btn-danger font-weight-bold px-3 d-flex align-items-center" target="_blank">
                <i class="fas fa-file-pdf mr-1"></i> Download PDF
              </a>
              <a href="<?= BASE_URL ?>laporan/mutasi_siswa_export_excel?<?= $query_string_custom ?>" class="btn btn-success font-weight-bold px-3 d-flex align-items-center">
                <i class="fas fa-file-excel mr-1"></i> Excel
              </a>
            </div>
          </div>
        ...
        <div class="simaks-paper-studio-container">
          <div class="simaks-paper-sheet">
            ...
            <div class="simaks-doc-title">
              LAPORAN REKAPITULASI MUTASI PESERTA DIDIK
            </div>
            <div class="simaks-doc-subtitle">
              Periode: <?= !empty($tanggal1) ? tgl_indo($tanggal1) . ' s/d ' . tgl_indo($tanggal2) : 'Semua Periode' ?>
            </div>
                ...
                <?php else: ?>
                  <?php $no = 1; foreach ($list as $d): ?>
                    <tr>
                      ...
                      <td style="text-align: center;"><?= tgl_indo($d['tanggal_mutasi']) ?></td>
                      <td style="font-weight: bold;"><?= htmlspecialchars($d['nama']) ?></td>
                      <td style="text-align: center; font-family: monospace;"><?= htmlspecialchars($d['nisn'] ?: '-') ?></td>
                      <td style="text-align: center;">Kelas <?= htmlspecialchars($d['nama_kelas'] ?? '-') ?></td>
                      <td style="text-align: center; font-weight: bold;"><?= htmlspecialchars($d['jenis_mutasi']) ?></td>
                      <td><?= htmlspecialchars($d['alasan'] ?: '-') ?></td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
